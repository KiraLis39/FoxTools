...
public class RomanArabicConverter {
	...
	public static int romanToArabic(String input) {
		...
		try {result = Integer.valueOf(input); // быть может, это уже и так арабский тип?
		} catch (Exception e) {			
			try {
			...
			} catch (Exception e2) {throw new RuntimeException("Не удалось преобразовать тип числа: " + e2.getMessage());}
		}
